...
export function InvestigationTableView({ 
  ...
  const totalActive = investigations.filter(inv => inv.status !== 'completed' && inv.status !== 'closed').length;
  const overdue = investigations.filter(inv => getDaysRemaining(inv.dueDate) < 0).length;
  const critical = investigations.filter(inv => inv.priority === 'critical').length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="mt-2">
            <div className="text-xs text-gray-500">
              {((totalActive / investigations.length) * 100).toFixed(1)}% of total
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="mt-2">
            <div className="text-xs text-gray-500">
              {overdue > 0 ? `${((overdue / investigations.length) * 100).toFixed(1)}% of total` : 'All on track'}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="mt-2">
            <div className="text-xs text-gray-500">
              {critical > 0 ? `${((critical / investigations.length) * 100).toFixed(1)}% of total` : 'No critical items'}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="mt-2">
            <div className="text-xs text-gray-500">
              {((completed / investigations.length) * 100).toFixed(1)}% completion rate
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center">
              <span className="text-purple-600 font-bold text-sm">{avgProgress}</span>
            </div>
          </div>
          <div className="mt-2">
            <ProgressBar progress={avgProgress} size="sm" showPercentage={false} color="blue" />
          </div>
        </div>
      </div>
